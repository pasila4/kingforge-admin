...
export async function listAdminByProducts(params: {
  page?: number;
  limit?: number;
  search?: string;
...
export async function createAdminByProduct(payload: CreateAdminByProductRequest) {
  const validated = CreateAdminByProductRequestSchema.safeParse(payload);
  if (!validated.success) throw new Error("Check the by-product details and try again.");
...
export async function updateAdminByProduct(id: string, payload: UpdateAdminByProductRequest) {
